...
if (!$is_member)
    if($lang == "") { //(기본)영문
        alert('Members-only service.', G5_BBS_URL.'/login.php?url='.urlencode($url));
    } else if ($lang == "ko") { //국문
...
if ($w == "d")
{
    ...
    if($row['mb_id'] != $member['mb_id'])
        if($lang == "") { //(기본)영문
            alert('You do not have permission to delete Wishlist products.');
        } else if ($lang == "ko") { //국문
...
else
{
    ...
    if(!$it_id)
        if($lang == "") { //(기본)영문
            alert('The product code is incorrect.', G5_SHOP_URL);
        } else if ($lang == "ko") { //국문
    ...
    if(! (isset($row['it_id']) && $row['it_id']))
        if($lang == "") { //(기본)영문
            alert('There is no product information.', G5_SHOP_URL);
        } else if ($lang == "ko") { //국문
