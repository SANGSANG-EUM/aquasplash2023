...
switch ($action) {
    ...
    case 'cart_update' : 
        ...
        {
            if($lang == "") { //(기본)영문
                die(json_encode(array('error' => "The operation cannot proceed any further.\n\nIt looks like you have set your browser's cookie acceptance to disabled.\n\nPlease enable cookie acceptance in your browser's Internet options.\n\nStill, the process will not proceed. If it does not work, please contact the shopping mall operator.")));
            } else if ($lang == "ko") { //국문
        ...
        {
            if($lang == "") { //(기본)영문
                die(json_encode(array('error' => 'You do not have permission to purchase the product.')));
            } else if ($lang == "ko") { //국문
        ...
        if ($count < 1)
            if($lang == "") { //(기본)영문
                die(json_encode(array('error' => 'Please select the products you want to add to your shopping cart.')));
            } else if ($lang == "ko") { //국문
        ...
        for($i=0; $i<$count; $i++) {
            ...
            if(!$it['it_id'])
                if($lang == "") { //(기본)영문
                    die(json_encode(array('error' => 'There is no product information.')));
                } else if ($lang == "ko") { //국문
            ...
            if($lst_count > 0 && !trim($_POST['io_id'][$it_id][$i]) && $_POST['io_type'][$it_id][$i] == 0)
                if($lang == "") { //(기본)영문
                    die(json_encode(array('error' => 'Please select the product option.')));
                } else if ($lang == "ko") { //국문
            ...
            for($k=0; $k<$opt_count; $k++) {
                ...
                if ($post_ct_qty < 1)
                    if($lang == "") { //(기본)영문
                        die(json_encode(array('error' => 'Please enter a quantity of 1 or more.')));
                    } else if ($lang == "ko") { //국문
            ...
            if($it['it_buy_min_qty'] || $it['it_buy_max_qty']) {
                ...
                if($it['it_buy_min_qty'] > 0 && $sum_qty < $it['it_buy_min_qty'])
                    if($lang == "") { //(기본)영문
                        die(json_encode(array('error' => 'Please order a total of '.number_format($it['it_buy_max_qty']).' or less options for '.$it['it_name'], './cart.   php')));
                    } else if ($lang == "ko") { //국문
            ...
            for($k=0; $k<$opt_count; $k++) {
                ...
                if($io_type) {
                    if((int)$io_price < 0)
                        if($lang == "") { //(기본)영문
                            die(json_encode(array('error' => 'Products with a negative purchase amount cannot be purchased.')));
                        } else if ($lang == "ko") { //국문
                ...
                if(isset($row2['ct_id']) && $row2['ct_id']) {
                    ...
                    {
                        if($lang == "") { //(기본)영문
                            die(json_encode(array('error' => "The stock quantity of " . $io_value . " is insufficient.\n\nCurrent stock quantity : " . number_format($tmp_it_stock_qty))));
                        } else if ($lang == "ko") { //국문
    ...
    case 'get_item_option' :
        ...
        if(!$it['it_use'] || $it['it_tel_inq'] || $is_soldout)
            if($lang == "") { //(기본)영문
                die(json_encode(array('error' => 'The product cannot be purchased.')));
            } else if ($lang == "ko") { //국문
    ...
    case 'wish_update' :
        ...
        if(! (isset($row['it_id']) && $row['it_id']))
            if($lang == "") { //(기본)영문
                die('There is no product information.');
            } else if ($lang == "ko") { //국문
                die('상품정보가 존재하지 않습니다.');
            }
        ...
        if (! (isset($row['wi_id']) && $row['wi_id'])) {
            ...
            if($lang == "") { //(기본)영문
                die('This product is already registered in your wishlist.');
            } else if ($lang == "ko") { //국문
                die('상위시리스트에 이미 등록된 상품입니다.');
            }
        }
