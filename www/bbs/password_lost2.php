...
if ($is_member) {
    alert_close('You are already logged in.', G5_URL);
    // alert_close('이미 로그인중입니다.hosu', G5_URL);
}

if (!chk_captcha()) {
    alert('The automatic registration prevention number is incorrect.');
    // alert('자동등록방지 숫자가 틀렸습니다.hosu');
}

$email = get_email_address(trim($_POST['mb_email']));

if (!$email)
    alert_close('There is an error in your email address.');
    // alert_close('메일주소 오류입니다.hosu');

$sql = " select count(*) as cnt from {$g5['member_table']} where mb_email = '$email' ";
$row = sql_fetch($sql);
if ($row['cnt'] > 1)
    alert('There are two or more identical email addresses.\\n\\nPlease contact the administrator.');
    // alert('동일한 메일주소가 2개 이상 존재합니다.\\n\\n관리자에게 문의하여 주십시오.hosu');
...
if (empty($mb['mb_id']) || $mb['mb_leave_date']) {
    ...
    // alert('존재하지 않는 회원입니다.hosu');
} elseif (is_admin($mb['mb_id'])) {
    alert('Administrator ID is not accessible.');
    // alert('관리자 아이디는 접근 불가합니다.hosu');
}
...
$subject = "[".$config['cf_title']."] This is an email with instructions for finding the member information you requested.";
// $subject = "[".$config['cf_title']."] 요청하신 회원정보 찾기 안내 메일입니다.hosu";
...
$content .= 'Guide to finding member information';
$content .= '</h1>';
...
$content .= addslashes($mb['mb_name'])." (".addslashes($mb['mb_nick']).")"." You made a request to find member information on ".G5_TIME_YMDHIS.".<br>";
$content .= 'Because even administrators of our site cannot know your password, we create a new password for you instead of providing you with your password.<br>';
$content .= 'Check the password to be changed below, then click the <span style="color:#ff3061"><strong>Change Password</strong> link.</span><br>';
$content .= 'When an authentication message is displayed stating that your password has been changed, enter your member ID and changed password on the homepage and log in.<br>';
$content .= 'After logging in, change your password to a new password in the Edit Information menu.';
$content .= '</p>';
...
alert_close($email.' An email has been sent to you to verify your member ID and password.\\n\\nPlease check your email.');
